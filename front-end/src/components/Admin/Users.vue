<template>
  <div>
    ...
    <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
      <h3 class="h6 mb-0">
        <i class="icon-people g-pos-rel g-top-1 g-mr-5"></i> 用户列表 <small v-if="users">(共 {{ users._meta.total_items }}
        个, {{ users._meta.total_pages }} 页)</small>
      </h3>
          ...
          <tr v-for="(user, index) in users.items" v-bind:key="index">
            ...
            <td>
              <router-link class="btn btn-xs u-btn-outline-purple"
                           v-bind:to="{ name: 'AdminEditUser', params: { id: user.id } }">编辑
              </router-link>
              <button class="btn btn-xs u-btn-outline-red" v-on:click="onDeleteUser(user)">删除</button>
            </td>
          </tr>
