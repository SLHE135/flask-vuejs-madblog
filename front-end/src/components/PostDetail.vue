<template>
  <div class="container">
    <!-- Modal: Edit Post -->
    <div id="editPostModal" aria-hidden="true" aria-labelledby="exampleModalCenterTitle" class="modal fade"
         role="dialog" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 id="editPostModalTitle" class="modal-title">Update Post</h5>
          ...
          <div class="modal-body">
            ...
            <form id="editPostForm" @submit.prevent="onSubmitUpdatePost" @reset.prevent="onResetUpdatePost">
              <div class="form-group" v-bind:class="{'u-has-error-v1': editPostForm.titleError}">
                <input id="editPostFormTitle" v-model="editPostForm.title" class="form-control" placeholder="标题"
                       type="text">
                <small v-show="editPostForm.titleError" class="form-control-feedback">{{
                    editPostForm.titleError
                  }}</small>
              </div>
              <div class="form-group">
                <input id="editPostFormSummary" v-model="editPostForm.summary" class="form-control" placeholder="摘要"
                       type="text">
              </div>
              <div class="form-group">
                <textarea id="editPostFormBody" v-model="editPostForm.body" class="form-control" placeholder=" 内容"
                          rows="5"></textarea>
                <small v-show="editPostForm.bodyError" class="form-control-feedback">{{
    ...
    <!-- Modal: Edit Comment -->
    <div id="editCommentModal" aria-hidden="true" aria-labelledby="exampleModalCenterTitle" class="modal fade"
         role="dialog" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 id="editCommentModalTitle" class="modal-title">Update Comment</h5>
            <button aria-label="Close" class="close" data-dismiss="modal" type="button">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            ...
            <form id="editCommentForm" @submit.prevent="onSubmitUpdateComment" @reset.prevent="onResetUpdateComment">
              <div class="form-group">
                <textarea id="editCommentFormBody" v-model="editCommentForm.body" class="form-control"
                          placeholder=" 评论内容" rows="5"></textarea>
                <small v-show="editCommentForm.bodyError" class="form-control-feedback">{{
      ...
      <div class="col-lg-9">
        ...
        <article class="g-mb-30 g-pt-15 g-pb-15">
          <header class="g-mb-30">
            ...
            <ul class="list-inline d-sm-flex g-color-gray-dark-v4 mb-0">
              <li v-if="post.author && post.author.id == sharedState.user_id" class="list-inline-item">
                <button class="btn btn-xs u-btn-outline-purple g-mr-5" data-target="#editPostModal" data-toggle="modal"
                        v-on:click="onEditPost(post)">编辑
                </button>
              </li>
              <li v-if="post.author && post.author.id == sharedState.user_id" class="list-inline-item">
                <button class="btn btn-xs u-btn-outline-red g-mr-5" v-on:click="onDeletePost(post)">删除</button>
              </li>
              <li class="list-inline-item">
                <a class="btn btn-xs u-btn-outline-aqua g-mr-10" href="#comment-list-wrap">评论</a>
              </li>
              <li v-if="post.author" class="list-inline-item">
                <router-link class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover g-text-underline--none--hover"
                             v-bind:to="{ path: `/user/${post.author.id}` }"><span
          ...
          <div id="like-post" class="row">
            <div class="col-lg-3">
              <button class="btn btn-block g-rounded-50 g-py-12 g-mb-10" v-bind:class="btnOutlineColor"
                      v-on:click="onLikeOrUnlikePost(post)">
                <i class="icon-heart g-pos-rel g-top-1 g-mr-5"></i> 喜欢<span
                v-if="post.likers_id && post.likers_id.length > 0"> | {{ post.likers_id.length }}</span>
              </button>
            </div>
            <div class="col-lg-9">
              <ul v-if="post.likers" class="list-inline mb-0">
                <li v-for="(liker, index) in post.likers"
                    v-bind:key="index" class="list-inline-item">
                  <router-link
                    ...
                    <img
                      ...
                      v-bind:alt="liker.name || liker.username" v-bind:src="liker.avatar">
                  </router-link>
                </li>
              </ul>
            </div>
        ...
        <div class="g-mb-60">
          <ul class="list-inline d-sm-flex mb-0">
            <li class="list-inline-item g-hidden-xs-down" data-placement="top" data-toggle="tooltip" title=""
                v-bind:class="{'u-pagination-v1__item--disabled': !post.prev_id}"
                v-bind:data-original-title="post.prev_title || ''">
              <router-link aria-label="Previous"
                           class="u-pagination-v1__item u-pagination-v1-4 g-brd-gray-light-v3 g-brd-primary--hover g-rounded-50 g-pa-7-16"
                           href="#" v-bind:to="{ name: 'PostDetail', params: { id: post.prev_id || 0 } }">
            ...
            <li class="list-inline-item ml-auto g-hidden-xs-down" data-placement="top" data-toggle="tooltip" title=""
                v-bind:class="{'u-pagination-v1__item--disabled': !post.next_id}"
                v-bind:data-original-title="post.next_title || ''">
              <router-link aria-label="Next"
                           class="u-pagination-v1__item u-pagination-v1-4 g-brd-gray-light-v3 g-brd-primary--hover g-rounded-50 g-pa-7-16"
                           href="#" router-link v-bind:to="{ name: 'PostDetail', params: { id: post.next_id || 0 } }">
                <span aria-hidden="true">
        ...
        <div id="comment-list-wrap" class="card border-0 g-mb-15">
          ...
          <div
            ...
            <div class="dropdown g-mb-10 g-mb-0--md">
              <span aria-expanded="false" aria-haspopup="true"
                    class="d-block g-color-primary--hover g-cursor-pointer g-mr-minus-5 g-pa-5" data-toggle="dropdown">
          ...
          <!-- Add Comment Form -->
          <form v-if="sharedState.is_authenticated" id="addCommentForm" class="g-mb-40"
                @submit.prevent="onSubmitAddComment" @reset.prevent="onResetAddComment">
            <div class="form-group">
              <textarea id="commentFormBody" v-model="commentForm.body" class="form-control" placeholder=" 写下你的评论 ..."
                        rows="5"></textarea>
              <small v-show="commentForm.bodyError" class="form-control-feedback">{{ commentForm.bodyError }}</small>
            </div>
            <button class="btn btn-secondary" type="reset">Cancel</button>
          ...
          <div v-else class="btn-group g-mr-10 g-mb-50 g-px-10">
            ...
            <div class="dropdown-menu"
              ...
              <router-link class="dropdown-item" v-bind:to="{ path: '/login', query: { redirect: $route.fullPath } }">
                站内账号
              </router-link>
              <div class="dropdown-divider"></div>
            ...
            <div v-for="(comment, index) in comments.items" v-bind:key="index">
              <div v-bind:id="'c' + comment.id"
                ...
                <div class="media-body">
                  <div class="g-mb-15">
                    <h5 v-if="comment.author.id == comment.post.author_id" class="h5 g-color-gray-dark-v1 mb-0">
                  ...
                  <ul class="list-inline d-sm-flex my-0">
                    <li v-if="!comment.disabled" class="list-inline-item g-mr-20">
                      <a class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover" href="javascript:;"
                         v-on:click="onLikeOrUnlikeComment(comment)">
                        <i class="icon-like g-pos-rel g-top-1 g-mr-3"
                           v-bind:class="{ 'g-color-red': comment.likers_id.indexOf(sharedState.user_id) != -1 }"></i>
                    ...
                    <ul class="list-inline mb-0 ml-auto">
                      <li class="list-inline-item g-mr-5" style="display: none;">
                        <button class="btn btn-xs u-btn-outline-purple" data-target="#editCommentModal"
                                data-toggle="modal" v-on:click="onEditComment(comment)">编辑
                        </button>
                      </li>
                      <li v-if="!comment.disabled && post.author.id == sharedState.user_id" class="list-inline-item">
                        <button class="btn btn-xs u-btn-outline-purple" v-on:click="onDisabledComment(comment)">屏蔽
                        </button>
                      </li>
                      <li v-if="comment.disabled && post.author.id == sharedState.user_id" class="list-inline-item">
                        <button class="btn btn-xs u-btn-outline-aqua" v-on:click="onEnabledComment(comment)">恢复</button>
                      </li>
                      <li v-if="comment.author.id == sharedState.user_id || post.author.id == sharedState.user_id"
                          class="list-inline-item">
                        <button class="btn btn-xs u-btn-outline-red" v-on:click="onDeleteComment(comment)">删除</button>
                      </li>
                    </ul>
                  </ul>
                </div>
              ...
              <!-- 子级评论，按时间正序排列 -->
              <div v-for="(child, cindex) in comment.descendants"
                ...
                <div class="media-body">
                  <div class="g-mb-15">
                    <h5 v-if="child.author.id == child.post.author_id" class="h5 g-color-gray-dark-v1 mb-0">
                      <router-link class="comment-author g-text-underline--none--hover"
                                   v-bind:to="{ path: `/user/${child.author.id}` }">
                  ...
                  <ul class="list-inline d-sm-flex my-0">
                    <li v-if="!child.disabled" class="list-inline-item g-mr-20">
                      <a class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover" href="javascript:;"
                         v-on:click="onLikeOrUnlikeComment(child)">
                        <i class="icon-like g-pos-rel g-top-1 g-mr-3"
                           v-bind:class="{ 'g-color-red': child.likers_id.indexOf(sharedState.user_id) != -1 }"></i>
                    ...
                    <li v-if="!child.disabled" class="list-inline-item g-mr-20">
                      <a class="comment-reply-link u-link-v5 g-color-gray-dark-v4 g-color-primary--hover"
                         href="javascript:;" v-on:click="onClickReply(child)">
                        <i class="icon-note g-pos-rel g-top-1 g-mr-3"></i>
                        回复
                      </a>
                    </li>
                    <ul class="list-inline mb-0 ml-auto">
                      <li class="list-inline-item g-mr-5" style="display: none;">
                        <button class="btn btn-xs u-btn-outline-purple" data-target="#editCommentModal"
                                data-toggle="modal" v-on:click="onEditComment(child)">编辑
                        </button>
                      </li>
                      <li v-if="!child.disabled && post.author.id == sharedState.user_id" class="list-inline-item">
                        <button class="btn btn-xs u-btn-outline-purple" v-on:click="onDisabledComment(child)">屏蔽
                        </button>
                      </li>
                      <li v-if="child.disabled && post.author.id == sharedState.user_id" class="list-inline-item">
                        <button class="btn btn-xs u-btn-outline-aqua" v-on:click="onEnabledComment(child)">恢复</button>
                      </li>
                      <li v-if="child.author.id == sharedState.user_id || post.author.id == sharedState.user_id"
                          class="list-inline-item">
                        <button class="btn btn-xs u-btn-outline-red" v-on:click="onDeleteComment(child)">删除</button>
                      </li>
                    </ul>
                  </ul>
                </div>
