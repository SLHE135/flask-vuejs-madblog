  ...
  <div>
    ...
    <div class="card border-0 g-mb-15">
      ...
      <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
        <h3 class="h6 mb-0">
          <i class="icon-bubbles g-pos-rel g-top-1 g-mr-5"></i> 与
          <router-link class="g-text-underline--none--hover" v-bind:to="{ path: `/user/${user.id}` }">
            {{ user.name || user.username }}
          </router-link>
          的对话 <small v-if="messages">(共 {{ messages._meta.total_items }} 条, {{ messages._meta.total_pages }} 页)</small>
        </h3>
        ...
        <span aria-expanded="false" aria-haspopup="true"
          ...
          <div class="dropdown-menu dropdown-menu-right rounded-0 g-mt-10">
            ...
            <router-link class="dropdown-item g-px-10" v-bind:to="{ name: 'MessagesIndexResource' }">
            ...
            <router-link class="dropdown-item g-px-10"
                         v-bind:to="{ path: $route.fullPath, query: { page: 1, per_page: 1 }}">
              <i class="icon-plus g-font-size-12 g-color-gray-dark-v5 g-mr-5"></i> 每页 1 条
            </router-link>
            <router-link class="dropdown-item g-px-10"
                         v-bind:to="{ path: $route.fullPath, query: { page: 1, per_page: 5 }}">
              <i class="icon-layers g-font-size-12 g-color-gray-dark-v5 g-mr-5"></i> 每页 5 条
            </router-link>
            <router-link class="dropdown-item g-px-10"
                         v-bind:to="{ path: $route.fullPath, query: { page: 1, per_page: 10 }}">
              <i class="icon-wallet g-font-size-12 g-color-gray-dark-v5 g-mr-5"></i> 每页 10 条
            </router-link>
            <router-link class="dropdown-item g-px-10"
                         v-bind:to="{ path: $route.fullPath, query: { page: 1, per_page: 20 }}">
          ...
          <div v-for="(message, index) in messages.items" v-bind:key="index">
            ...
            <section v-if="message.sender.id != sharedState.user_id" class="g-mb-30">
              <div class="media g-mb-12">
                ...
                <router-link class="d-flex align-self-end g-mr-12" v-bind:to="{ path: `/user/${message.sender.id}` }">
                <span v-if="message.is_new" class="d-inline-block g-pos-rel">
                  ...
                  <img class="rounded-circle g-width-36 g-height-36 g-brd-around g-brd-gray-light-v4 g-pa-2"
                       v-bind:alt="message.sender.name || message.sender.username"
                       v-bind:src="message.sender._links.avatar">
                </span>
                  <img v-else class="rounded-circle g-width-36 g-height-36 g-brd-around g-brd-gray-light-v4 g-pa-2"
                       v-bind:alt="message.sender.name || message.sender.username"
                       v-bind:src="message.sender._links.avatar">
                </router-link>
                ...
                <div class="media-body">
                  <div
                    class="d-inline-block g-width-300 g-width-auto--sm g-bg-gray-light-v8 g-font-size-12 g-font-size-default--lg g-color-gray-dark-v6 g-rounded-10 g-pa-10-15">
                    <p class="mb-0">
            ...
            <section v-else class="g-mb-30">
              <div class="media g-mb-12">
                ...
                <div class="ml-auto">
                  <div
                    class="d-inline-block g-width-300 g-width-auto--sm g-bg-lightblue-v6 g-font-size-12 g-font-size-default--lg g-color-gray-dark-v6 g-rounded-10 g-pa-10-15">
                    <p class="mb-0">
                      <!-- vue-markdown 开始解析markdown，它是子组件，通过 props 给它传值即可
                      v-highlight 是自定义指令，用 highlight.js 语法高亮 -->
                      <vue-markdown
                ...
                <!-- Chat. Message Area. Message. Avatar. -->
                <router-link class="d-flex align-self-end g-ml-12" v-bind:to="{ path: `/user/${message.sender.id}` }">
                <span v-if="message.is_new" class="d-inline-block g-pos-rel">
                  <span class="u-badge-v2--xs u-badge--top-left g-bg-red g-mt-7 g-ml-7"></span>
                  <img class="rounded-circle g-width-36 g-height-36 g-brd-around g-brd-gray-light-v4 g-pa-2"
                       v-bind:alt="message.sender.name || message.sender.username"
              ...
              <!-- Chat. Message Area. Message Time -->
              <em
                class="d-flex align-self-center align-items-center justify-content-end g-font-style-normal g-color-gray-light-v1">
                <i class="hs-admin-time icon-clock g-mr-5"></i>
                <small>{{ $moment(message.timestamp).format('YYYY年MM月DD日 HH:mm:ss') }}</small>
                <i v-if="!message.is_new" class="icon-check g-font-size-10 g-color-lightblue-v3 g-ml-12"></i>
              </em>
    ...
    <!-- Reply Message Form -->
    <form id="replyMessageForm" class="g-mb-40" @submit.prevent="onSubmitReply" @reset.prevent="onResetReply">
      <div class="form-group">
        <textarea id="replyMessageFormBody" v-model="replyMessageForm.body" class="form-control" placeholder=" 悄悄话..."
                  rows="5"></textarea>
        <small v-show="replyMessageForm.bodyError" class="form-control-feedback">{{
            replyMessageForm.bodyError
          }}</small>
      </div>
