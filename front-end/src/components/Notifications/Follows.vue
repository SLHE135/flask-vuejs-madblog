  ...
  <div>
    <div class="card border-0 g-mb-15">
      ...
      <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
        <h3 class="h6 mb-0">
          <i class="icon-people g-pos-rel g-top-1 g-mr-5"></i> All Follows <small v-if="followers">(共
          {{ followers._meta.total_items }} 个, {{ followers._meta.total_pages }} 页)</small>
        </h3>
      ...
      <div v-if="followers" class="card-block g-pa-0">
        ...
        <div v-for="(follower, index) in followers.items"
             v-bind:key="index"
          ...
          <div class="g-mt-2">
            <router-link v-bind:to="{ path: `/user/${follower.id}` }">
              <img class="g-width-50 g-height-50 rounded-circle mCS_img_loaded"
                   v-bind:alt="follower.name || follower.username" v-bind:src="follower._links.avatar">
            </router-link>
          </div>
          <div class="align-self-center g-px-10">
            <h5 class="h5 g-color-gray-dark-v1 mb-0">
              <router-link class="g-text-underline--none--hover" v-bind:to="{ path: `/user/${follower.id}` }">
                <span class="g-mr-5">{{ follower.name || follower.username }}</span>
              </router-link>
              <small class="g-font-size-12 g-color-deeporange">{{ follower.followers_count }} followers</small>, <small
              class="g-font-size-12 g-color-aqua">{{ follower.followeds_count }} following </small>
              <span class="h6">关注了你</span>
            </h5>
            <p class="m-0">{{ $moment(follower.timestamp).format('YYYY年MM月DD日 HH:mm:ss') }}</p>
          </div>
          <div class="align-self-center ml-auto">
            <button v-if="follower.is_following" class="btn btn-block u-btn-outline-red g-rounded-20 g-px-10"
                    v-on:click="onUnfollowUser(follower)">Unfollow
            </button>
            <button v-if="!follower.is_following && follower.id != sharedState.user_id"
                    class="btn btn-block u-btn-outline-primary g-rounded-20 g-px-10"
                    v-on:click="onFollowUser(follower)">Follow
            </button>
          </div>
        </div>
