  ...
  <div>
    ...
    <div class="card border-0 g-mb-15">
      ...
      <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
        <h3 class="h6 mb-0">
          <i class="icon-bubbles g-pos-rel g-top-1 g-mr-5"></i> Received Messages <small v-if="messages">(共
          {{ messages._meta.total_items }} 条, {{ messages._meta.total_pages }} 页)</small>
        </h3>
      ...
      <!-- Panel Body -->
      <div v-for="(message, index) in messages.items"
           v-bind:key="index"
           class="d-flex justify-content-start g-brd-around g-brd-gray-light-v4 g-brd-left-1 g-pa-20 g-mb-10">
        <div class="g-mt-2">
          <router-link v-bind:to="{ path: `/user/${message.sender.id}` }">
            ...
            <img v-else class="g-brd-around g-brd-gray-light-v4 g-pa-2 g-width-50 g-height-50 rounded-circle"
                 v-bind:alt="message.sender.name || message.sender.username" v-bind:src="message.sender._links.avatar">
          </router-link>
        </div>
        <div class="align-self-center g-px-10">
          <h5 class="h5 g-color-gray-dark-v1 mb-0">
            <router-link class="g-text-underline--none--hover" v-bind:to="{ path: `/user/${message.sender.id}` }">
              <span class="g-mr-5">{{ message.sender.name || message.sender.username }}</span>
            </router-link>
            <small class="g-font-size-12 g-color-aqua">给你发送了<small v-if="message.new_count"
                                                                   class="g-font-size-12 g-color-deeporange">
              {{ message.new_count }} 条新 </small>私信</small>
          </h5>
        ...
        <ul class="list-inline mb-0 align-self-center ml-auto">
          <li v-if="!message.is_blocking" class="list-inline-item g-mr-5">
            <button class="btn btn-block u-btn-outline-red g-rounded-20 g-px-10"
                    v-on:click="onBlock(message.sender.id)">拉黑
            </button>
          </li>
          <li v-else class="list-inline-item g-mr-5">
            <button class="btn btn-block u-btn-outline-aqua g-rounded-20 g-px-10"
                    v-on:click="onUnblock(message.sender.id)">取消拉黑
            </button>
          </li>
