  ...
  <div>
    ...
    <div class="card border-0 g-mb-15">
      ...
      <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
        <h3 class="h6 mb-0">
          <i class="icon-bubbles g-pos-rel g-top-1 g-mr-5"></i> Recived Comments Likes <small v-if="likes">(共
          {{ likes._meta.total_items }} 条, {{ likes._meta.total_pages }} 页)</small>
        </h3>
      ...
      <div v-if="likes" class="card-block g-pa-0">
        ...
        <div v-for="(like, index) in likes.items"
             v-bind:key="index" class="media g-brd-around g-brd-gray-light-v4 g-pa-30 g-mb-20">
          <router-link v-bind:to="{ path: `/user/${like.user.id}` }">
            <span v-if="like.is_new" class="d-inline-block g-pos-rel">
              ...
              <img
                class="d-flex g-brd-around g-brd-gray-light-v3 g-pa-2 g-width-40 g-height-40 rounded rounded-circle mCS_img_loaded g-mt-3 g-mr-15"
                v-bind:alt="like.user.name || like.user.username" v-bind:src="like.user._links.avatar">
            </span>
            <img v-else
                 class="d-flex g-brd-around g-brd-gray-light-v3 g-pa-2 g-width-40 g-height-40 rounded rounded-circle mCS_img_loaded g-mt-3 g-mr-15"
                 v-bind:alt="like.user.name || like.user.username" v-bind:src="like.user._links.avatar">
            ...
            <div class="g-mb-15">
              <h5 class="h5 g-color-gray-dark-v1 mb-0">
                <router-link class="comment-author g-text-underline--none--hover"
                             v-bind:to="{ path: `/user/${like.user.id}` }">{{ like.user.name || like.user.username }}
                </router-link>
                <span class="h6">点赞了你的 <router-link class="g-text-underline--none--hover"
                                                    v-bind:to="{ path: `/post/${like.comment.post.id}#c${like.comment.id}` }"> 评论 </router-link></span>
              </h5>
              <span class="g-color-gray-dark-v4 g-font-size-12">{{
                  $moment(like.timestamp).format('YYYY年MM月DD日 HH:mm:ss')
                }}</span>
            </div>
              ...
              <li v-if="!like.comment.disabled" class="list-inline-item g-mr-20">
                <span class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover">
                  <i class="icon-like g-pos-rel g-top-1 g-mr-3"
                     v-bind:class="{ 'g-color-red': like.comment.likers_id.indexOf(sharedState.user_id) != -1 }"></i>
