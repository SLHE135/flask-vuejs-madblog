export default {
  ...
  state: {
    ...
    //后端返回的是一个 base64 编码的字符串，需要解码
    // user_avatar: window.localStorage.getItem('madblog-token') ? JSON.parse(atob(window.localStorage.getItem('madblog-token').split('.')[1])).user_avatar : '',
    //user_avatar进行解码
    user_avatar_decode: window.localStorage.getItem('madblog-token') ? atob(JSON.parse(atob(window.localStorage.getItem('madblog-token').split('.')[1])).user_avatar) : '',
